...
void raiseError(const char *const s){ printf("ERROR: %s\n", s); exit(1); }
void raiseError(const char *const s0, const char *const s1, const char *const s2){ printf("ERROR: %s%s%s\n", s0, s1, s2); exit(1); }
...
Repeat:
	...
	case Token::Type::Integral:{
			...
			/* CALCULATIONS */{
				...
				runInfo.nodeIndex = funcNodeIndex;
				double sum = (funcResult + recCalculate(functions, runInfo)) * sp::QuadratureWeights[0];
